...
- `/_astro/...` 是 Astro 构建生成的静态资源；
- `/pagefind/pagefind.js` 是站内搜索资源；
- `/assets/music/track-01.flac` 是音频资源；
- `200` 表示资源成功返回；
- `206` 表示音频文件被分段请求。

这说明该域名可能正在访问或引用当前站点资源。
...
```text
www.xinlanting.com -> 119.28.14.125
```

说明该域名的 `www` 记录解析到了当前服务器 IP。
...
但注册人姓名、电话和邮箱没有公开，因此无法直接确认真实身份。

处理方式：

1. 确认主域名正常访问；
2. 使用 Nginx 限制未知 Host；
3. 对未知域名返回 `444`；
4. 执行 `sudo nginx -t` 检查配置；
5. 配置检查通过后执行 `sudo systemctl reload nginx`；
6. 使用 `curl` 验证结果。
...
如果返回空响应、连接断开或无法正常显示站点内容，说明未知 Host 限制已经生效。
...
合理处理方式是限制 Host，而不是只关注“是谁指向了服务器”。
